...
In [tutorial 1](./tutorial1-use-existing-components.md), you learned how to use existing components in the gallery. What if the component you want is not in the gallery, but you have the local code with the similar functionality? The suggested way is to create your own component. Then you can reuse it anytime and share it to the gallery for others who have the same needs. This tutorial will help you on the step by step to create a new component from your local code.

## Predefinition on the scenario
Let's continue with the NYC taxi fare prediction case in [tutorial 1](./tutorial1-use-existing-components.md) to create a new component. Assume we do not have existing nyc-taxi-fare-prediction components in the gallery, but instead, we have below local code to train NYC taxi fare prediction model by using `xgb.XGBRegressor`. 
...
Right now, we plan to create XGBRegressor component which includes the core training/evaluation functions. Then the new component could be easily reused for training new model with the refreshed data or other prediction pipeline. 
...
1. Prepare the Yaml Spec for the new component. This includes several sub-steps
...
2. Upgrade local code to be compatible with Yaml Spec. This includes several sub-steps
    1. Import necessary Azure Machine Learning packages.
    2. Parse all inputs from args and upgrade core functions with the variable inputs.
    3. Manage the output of component.

### Create Yaml Spec 
In this section, we will go through 4 steps to create Yaml Spec for our new XGBRegressor component. But this example may not cover all the available settings and their constraints. So we strongly recommend you to read [component spec definition](../component-spec-definition.md) for more details before you create your own component.

#### Step 1 - Metadata
Metadata is the starting point to create the component to define the basis (e.g., name, version) and its extensible tags. In our XGBRegressor component, let's use the 'microsoft.com.azureml.samples.XGBRegressor' as the name and 'XGBRegressor' as the display name for better experience on UI. To facilitate others on the understanding of this component, we could also add tags like 'Tutorial' to this component.
...
##### Inputs
When we define our XGBRegressor component __data inputs__, We would assume the data which flows into our component has already been cleaned and split by the precedent components. Then we need to declare 4 data inputs:
- Feature table which will be used for training.
- Label vector which will be used for training.
- Feature table which will be used for evaluation.
- Label vector which will be used for evaluation.
...
Each of parameters is defined like the following sample Yaml under `'inputs'` setting:
...
```
#### Step 4 - Envrionment.
`'environment'` in Yaml spec is used for defining your running environment includes docker image, python Conda config and operating system(i.e. os). In our XGBRegressor component, we use the default azureml docker image and Linux os. For conda environment, we leverage the default Azure Machine Learning conda config but add __xgboost__ as an extra dependency for our environment:
...
### Upgrade Py Code
To upgrade local py code to compatible component py code need follow 3 major steps:
...
Based on the local python code and our component interface definition, here is the full Py code for XGBRegressor component:
```python
import os
import sys
import argparse
...
When the Yaml Spec and Py code are ready, we could follow the major register guidance in [tutorial 1](./tutorial1-use-existing-components.md) to register our XGBRegressor component through Azure Machine Learning Component page. The only difference is that you need register the component from 'Local files' and choose the right path where contains the Yaml Spec and Py code:
